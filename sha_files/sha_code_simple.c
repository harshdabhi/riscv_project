...
double calculate_utilization() {
    ...
    double user_time = usage.ru_utime.tv_sec + usage.ru_utime.tv_usec / 1.0e6;
    double sys_time = usage.ru_stime.tv_sec + usage.ru_stime.tv_usec / 1.0e6;

    // Get the wall-clock time (real time)
    struct timeval start_time, end_time;
    gettimeofday(&start_time, NULL);

    // Simulate some work here (replace with your actual program logic)
    for (int i = 0; i < 100000000; ++i);  // Busy work loop

    gettimeofday(&end_time, NULL);

    // Calculate wall-clock time
    double real_time = (end_time.tv_sec - start_time.tv_sec) + (end_time.tv_usec - start_time.tv_usec) / 1.0e6;

    // Calculate CPU utilization
    double total_cpu_time = user_time + sys_time;
    double cpu_utilization = (total_cpu_time / real_time) * 100;
    ...
    printf("Total RAM available: %ld MB\n", total_memory);
...
int main() {
    ...
    const char *data = "hello world";  // Example input
    uint32_t state[8];
    ...
    sha256_init(state);
    sha256_update(state, (const uint8_t *)data, strlen(data));
    sha256_final(state, (const uint8_t *)data, strlen(data), hash);
    ...
    // Output the SHA-256 hash in hexadecimal format
    for (int i = 0; i < SHA256_BLOCK_SIZE; i++) {
        printf("%02x", hash[i]);
    }
    printf("\n");
